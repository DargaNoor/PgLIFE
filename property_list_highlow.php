...
<html lang="en">
    ...
    <body>
        ...
        <div class="page-container">
            <div class="filter-bar row justify-content-around">
                ...
                <form action="property_list_highlow.php" name="form" id="form" method="GET">
                    <?php if(isset($_GET['sort'])):
                            if($_GET['sort']=='high-rent'):?>
                                <input type="radio" id="high" name="sort" value="high-rent" checked />
                                <label for="high">Highest Rent First</label>
                                <input type="radio" id="low" name="sort" value="low-rent" />
            ...
            if($filter!="none1"):
                if(isset($_GET['sort'])):
                    if("high-rent"==$_GET['sort']):
                        $sql_2 = "SELECT p.* FROM properties p,cities c WHERE p.city_id = c.id and c.name='$city_name' and p.gender='$filter' order by rent DESC";
                    else:
                        if("low-rent"==$_GET['sort']):
                            $sql_2 = "SELECT p.* FROM properties p,cities c WHERE p.city_id = c.id and c.name='$city_name' and p.gender='$filter' order by rent ASC";
                        endif;
                    endif;
                else:
                    $sql_2 = "SELECT p.* FROM properties p,cities c WHERE p.city_id = c.id and c.name='$city_name' and p.gender='$filter' order by rent ASC";
                ...
                if(isset($_GET['sort'])):
                    if($_GET['sort']=="high-rent"):
                        $sql_2 = "SELECT p.* FROM properties p,cities c WHERE p.city_id = c.id and c.name='$city_name' order by rent DESC";
                    else:
                        if($_GET['sort']=="low-rent"):
            ...
            <?php if(isset($_SESSION['user_id'])): ?>
                <p><a href="api/form.php" id="click" style="font-size: 22px;text-decoration: none;color:green;">Click</a> [If you want to change the Interested PG ones]<?php endif;?></p>
